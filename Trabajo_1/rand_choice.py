...
class Job:
    def __init__(self, mach_req, proc_time, number):
        ...
        self.processes = [i + number*len(mach_req) for i in range(len(mach_req))]
...
def shortest_job(jobs, mach):
    ...
    job_next = np.random.choice(job_mach_free)
...
def eleccion_aleatoria(jobs, machines, iterations=100):
    best = 10000000000000
    for i in range(iterations):
        jobs_copy = [Job(job.mach_req, job.proc_time, job.number) for job in jobs]
        machines_copy = [Machine(machine.number) for machine in machines]
        ans, machine_order = aleatorio(jobs_copy, machines_copy)
        if max([x["End"] for x in ans]) < best:
            best = max([x["End"] for x in ans])
            best_ans = ans
            best_order = machine_order
    return max([x["End"] for x in best_ans]), best_order