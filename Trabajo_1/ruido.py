...
def aleatorio(jobs, machines, iterations=100):
    best = 10000000000000
    for i in range(iterations):
        ...
        ans, machine_order, process_order = order(jobs_copy, machines_copy)
        if max([x["End"] for x in ans]) < best:
            best = max([x["End"] for x in ans])
            best_ans = ans
            best_order = machine_order
            best_process_order = process_order
    return max([x["End"] for x in best_ans]), best_order, best_process_order
