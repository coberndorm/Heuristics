...
# Original code to turn the vector back into a matrix, changed for a better one, think it is working
#def sol_vector_to_matrix(machine_order_vector, machines_required, n ,m):
    ...
    machine_order_matrix = [[get_key(processes_num,machine_order_vector[i * m + j])[0] for i in range(n)] for j in range(m)]
